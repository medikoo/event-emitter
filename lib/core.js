...
on = function (type, listener) {
	...
	else if (data[type].hasOwnProperty('__eecol__')) data[type].push(listener);
	else (data[type] = [data[type], listener]).__eecol__ = true;
...
off = function (type, listener) {
	...
	if (!this.hasOwnProperty('__ee__')) return this;
	data = this.__ee__;
	if (!hasOwnProperty.call(data, type)) return this;
	listeners = data[type];

	if (listeners.hasOwnProperty('__eecol__')) {
...
emit = function (type) {
	...
	if (!this.hasOwnProperty('__ee__')) return;
	data = this.__ee__;
